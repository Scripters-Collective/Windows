...
# Set output path to network share with hostname and date
$networkShare = "\\SERVER\Share\HardwareReports"
...
if (-not (Test-Path $networkShare)) {
    ...
    Write-Host "Please verify the path exists and you have write permissions." -ForegroundColor Red
    exit 1
}
...
# Write to file / fallback if network share is unreachable
try {
    $report | Out-File -FilePath $outputPath -Encoding UTF8 -ErrorAction Stop
    Write-Host "`nHardware information successfully saved to: $outputPath" -ForegroundColor Green
}
catch {
    Write-Host "`nFailed to write to network share: $_" -ForegroundColor Red
    $fallbackPath = "C:\Temp\$fileName"
    Write-Host "Attempting to save locally to: $fallbackPath" -ForegroundColor Yellow
    try {
        if (-not (Test-Path "C:\Temp")) { New-Item -ItemType Directory -Path "C:\Temp" -Force | Out-Null }
        $report | Out-File -FilePath $fallbackPath -Encoding UTF8 -ErrorAction Stop
        Write-Host "Hardware information saved to fallback location: $fallbackPath" -ForegroundColor Green
    }
    catch {
        Write-Host "Failed to write to fallback location: $_" -ForegroundColor Red
    }
}
